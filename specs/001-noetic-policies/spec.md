...
- What happens when progress conditions evaluate outside 0.0–1.0? → Runtime warning from kernel; static detection deferred to Phase 2 (requires interval arithmetic on CEL expressions)
- How does the system handle conflicting temporal bounds between policy and goal level? → Goal-level bounds cannot exceed policy-level bounds; validation (FR-008h) rejects policies with error code E-TEMPORAL-HIERARCHY when goal deadlines/timeouts/max_steps exceed the global limit
...
**Testing Requirements** (per constitution Principle IV): All implementation must include Unit tests (core functionality), Integration tests (end-to-end workflows), Property-based tests (validation rules), and Comparison tests (stdlib policies vs reference implementations).
...
- **FR-007**: System MUST validate that goal states are reachable from initial states and SHOULD report minimum-cost path length (using transition costs) to each reachable goal (in both fast and thorough modes)
...
- **FR-013**: System MUST support deterministic constraint evaluation semantics with configurable restriction modes (safe subset default: deterministic operations only as defined in docs/cel-guide.md; full CEL optional: complete standard library; extension points for custom deterministic functions). "External calls" means any CEL function not in the deterministic subset.
...
- **FR-016**: System MUST support both fast validation mode (schema validation FR-002, constraint syntax checking FR-003, transition well-formedness FR-006, basic reachability FR-004 using shallow BFS - completing in <1 second for policies ≤100 states) and thorough validation mode (all fast mode checks plus cycle detection, deadlock detection FR-005, invariant consistency verification, scoring consistency, temporal feasibility, comprehensive edge case analysis - no time constraint)
- **FR-017**: System MUST monitor resource consumption during validation and issue warnings when CPU time or memory usage exceeds configurable thresholds (only CPU time and memory monitored in Phase 1; defaults: 5 seconds CPU time, 1 GB memory)
- **FR-018**: System MUST provide comprehensive logging of all validation operations including which checks were performed, their results, and timing information (always enabled, no configuration to disable per clarification Q3)
...
- **State Graph** (concept): A directed graph of abstract states and transitions representing all possible execution paths. Each state has preconditions, each transition has effects. Abstract states are templates; runtime execution creates concrete state snapshots. In YAML: `state_graph` field; in Python: `StateGraph` Pydantic model.
- **State**: A node in the state graph representing an abstract configuration. Has a unique name and optional state preconditions (conditions that must hold to enter this state). Distinguished from state snapshots (concrete runtime values).
- **Transition**: An edge in the state graph connecting two states. Has transition preconditions (conditions that must hold to take this transition), effects (what changes in state variables), and an optional cost weight for pathfinding optimization. If both `cost` (static float) and `cost_expr` (dynamic CEL) are specified, `cost_expr` takes precedence; static `cost` is ignored with validation warning.
...
- **Transition Cost**: A numeric weight (static or dynamic via CEL expression) on a transition representing resource expenditure. Used by the kernel's planner to find cost-optimal paths to goals. See FR-008d for validation rules.
- **Goal Priority**: An integer ordinal ranking on goal states (higher = more preferred). When multiple goals are achievable, priority determines which the agent pursues first. See FR-008e for validation rules.
- **Goal Reward**: A positive float value representing the cardinal utility of achieving a goal. Used by the kernel's planner for utility maximization. See FR-008e for validation rules.
